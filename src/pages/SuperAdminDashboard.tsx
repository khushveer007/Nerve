...
import { BRANDING_TYPES, CONTENT_TYPES } from '@/lib/constants'

const TEAM_STYLE = {
  branding: {
    bg50: 'bg-pink-50', bg100: 'bg-pink-100',
    text600: 'text-pink-600', text700: 'text-pink-700',
    bar: 'bg-pink-500', badge: 'bg-pink-100 text-pink-700',
  },
  content: {
...
function TeamContent({
  ...
  const label = team === 'branding' ? 'Branding' : 'Content'
  ...
  const teamEntries = allEntries.filter(e =>
    (team === 'branding' ? BRANDING_TYPES : CONTENT_TYPES as readonly string[]).includes(e.type)
  )
      ...
      <div className="grid grid-cols-5 gap-5">
        ...
        <div className="col-span-3 hub-card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-foreground">{label} Team Members</h2>
            <Link to={`/${team}/team`} className="text-xs text-primary hover:underline">View all</Link>
          </div>
                ...
                <div key={u.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className={`w-7 h-7 rounded-full ${s.bg100} flex items-center justify-center shrink-0`}>
                      <span className={`text-xs font-semibold ${s.text700}`}>
          ...
          {teamUsers.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-6">No members assigned yet.</p>
          )}
        </div>
...
export default function SuperAdminDashboard() {
  ...
  const brandingEntries = useMemo(() =>
    entries.filter(e => (BRANDING_TYPES as readonly string[]).includes(e.type)), [entries])
    ...
    <div className="animate-fade-in space-y-5">
      ...
      <Tabs defaultValue="overview" className="w-full">
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <div className="hub-card">
              ...
              <div className="space-y-5">
                {[
                  { team: 'branding', label: 'Branding Team', bar: 'bg-pink-500', entries: brandingEntries.length, link: '/branding/team' },
                  { team: 'content',  label: 'Content Team',  bar: 'bg-blue-500', entries: contentEntries.length,  link: '/content/team' },
                ].map(t => (
                  <div key={t.team} className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-medium text-foreground">{t.label}</span>
                      <div className="flex items-center gap-3">
                        <span className="text-muted-foreground">{t.entries} entries</span>
                    ...
                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                      <div className={`h-full rounded-full ${t.bar} transition-all duration-500`}
                        style={{ width: entries.length > 0 ? `${(t.entries / entries.length) * 100}%` : '0%' }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        <TabsContent value="branding">
          <TeamContent team="branding" allUsers={users} allEntries={entries} />
        </TabsContent>

        {/* ── Content Team ── */}
        <TabsContent value="content">
          <TeamContent team="content" allUsers={users} allEntries={entries} />
        </TabsContent>
      </Tabs>
    </div>
  )
